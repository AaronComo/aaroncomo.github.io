...
title: "高精度大数运算系统"
authors: Weifeng Liu
date: 2021-01-27 20:12:00 +0800
categories: [Software]
tags: []
pin: false
---

项目地址: [GitHub](https://github.com/AaronComo/large-number-arithmetic-system)

效果展示: [bilibili](https://www.bilibili.com/video/BV1kr4y1v7AT)

## 前言

在科学技术迅速发展的背景下, 矩阵运算与大数运算开始成为了计算机运算的重点. 然而 C 语言被发明出来的时代并没有如此的需求, 这就导致 C 语言直到当今最大只能支持 `long long` 类型的整数, 这对于大数运算十分不利.

本项目通过构建高精度大整数运算系统来缓解这个问题. 项目使用动态数组来支持任意位数的大整数进行四则运算, 能够按照标准计算式的方式进行输入.

## 特别声明

- 本项目引入 `Windows.h` 来构建基础 UI 界面, 目前只支持 Windows 设备.
- 文件中的中文使用编码 GB2312.
...
- 简化运算并统一后续运算时中间变量的扩展方式
- 将大数放在 `array1` 中, 小数放在 `array2` 中
...
~~~c
oid multiply(Array array1, Array array2)
{
	int i = 0, j = 0, k = 0;
	char num = '\0';
	int max_size = 0;
